...
def run_rag_pipeline():
    ...
    # target
    target_sample_size = min(3, len(filtered_targets))
    target_queries = random.sample(filtered_targets, target_sample_size)
    
    
    sampled_queries = target_queries
    
    print(f"\n [보고서용 추출 완료] 타겟 케이스 {len(target_queries)}건 : (총 {len(sampled_queries)}건)")

    report_dir = "../testcase_report"
